...
use app\common\service\NewsIndex as S;
...
class Index extends \app\admin\controller\Base {
	...
	//	$this->category = MC::order('sort', 'asc')->where("model", 1)->where("status", 1)->select()->toArray();
		
		$this->category =  get_tree(MC::order('sort', 'asc')->where("model", 1)->where("status", 1)->select()->toArray());
