...
class Pic extends \app\BaseController {
	...
	public function index($id = '') {
		...
		$list = Db::name('images_index')->where('status', 1)->where($where)->order('sort', 'asc')->paginate(12);
		$page = $list->render();
	...
	public function show($id = '') {
		...
		$model = Db::name('images_index')->where('id', $id)->find();
	...
	if(count($category)==0)
		...
		$front = Db::name('images_index')->where($where)->where("id", ">", $id)->where('status', 1)->find();
		$after = Db::name('images_index')->where($where)->where("id", "<", $id)->where('status', 1)->find();
