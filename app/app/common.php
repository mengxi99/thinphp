       ...
       layui.use(["jquery"],function() {
        ...
        $("#' . $val . '").on("click", function () {
            layer.open({
                ...
                shade: 0.1,
                area: screen(),
                content:"/admin.php/index/optPhoto",
 ...
 * @return array|string
 */
function cfg($name, $default = '', $path = 'cfg') {
	return \think\facade\Config::get('web.' . $name, $default);
}

function getnav($tbname = '') {

	$nav = \think\facade\Db::name($tbname)->where('status', 1)->order('sort asc')->select()->toarray();

	if ($tbname == 'page_index') {
		$nav = \think\facade\Db::name($tbname)->where('status', 1)->where('c_id', 0)->order('sort asc')->select()->toarray();
	}

	foreach ($nav as $k => $v) {

		$nav[$k]['url'] = "/" . str_ireplace('_category', '', str_ireplace('_index', '', $tbname)) . "/" . $v['id'] . ".html";
	}
...
function cid($id = '') {
	...
	$list = \think\facade\Db::name('page_index')->where('c_id', $id)->order('sort asc')->select();
	return $list;
}
