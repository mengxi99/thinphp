...
use app\common\model\PageIndex as M;
use app\common\service\PageIndex as S;
...
class Index extends \app\admin\controller\Base {
	...
	public function initialize() {
		parent::initialize();
	//	$this->category = MC::order('sort', 'asc')->where("model", 3)->where("status", 1)->select()->toArray();
		$this->category =  get_tree(MC::order('sort', 'asc')->where("model", 3)->where("status", 1)->select()->toArray());
	}
